...
class PDModel(object):
    ...
    def __call__(self, fluxP, fluxD):
        ...
        fluxP = np.tensordot(fluxP, self.betaF, axes=([1],[0]))
        fluxD = np.tensordot(fluxD, self.betaF, axes=([1],[0]))
        self
        # Smearing Rself
        fluxP = np.tensordot(fluxP, self.rgdtF, axes=([1],[0]))
        fluxD = np.tensordot(fluxD, self.rgdtF, axes=([1],[0]))
